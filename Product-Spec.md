...
| REQ-5 | 治理 CLI（零依赖 Node≥18） | node .zcode/zbase.mjs 全 verb 可用；无 npm 依赖 |
...
| REQ-14 | Fast Mode 贷款语义 | --minutes 必填封顶 8h + --reason 必填；仅跳 allowFastSkip 预标记项；SKIPPED 留痕；DEBT 阻断 task finish/release 直到还清 |
...
| REQ-19 | 记忆归档 | archive --apply：Done/Notes 超额移 progress.archive.md（append-only+指针）；ledgerHealth 建议 |
...
| REQ-24 | 发布证明聚合 | release 九条件（dod/trace/ledger/receipt fresh/fast 清欠/backlog/sync）READY 才 exit 0，never tag；dod 十二步静态 DoD |
...
| REQ-29 | CI workflow | .github/workflows/gate.yml：矩阵（linux×win × node）+ selftest 先行 + doctor always（诊断不阻断） |
...
| REQ-32 | 单目录封装 | 脚手架本体全收 `.zcode/`（引擎/skills/commands/feedback/rules/docs/harness/githooks/state）；install=复制一个目录+根级种子；运行态 .zcode/state/ gitignored |
| REQ-33 | 三缝执法 | ZCode 会话 hooks（7 事件）+ git hooks（pre-commit/commit-msg/pre-push 经 core.hooksPath）+ CI workflow——会话内、人提交、推送前三个层面全覆盖 |
| REQ-34 | 自我插桩（超越层） | 检查有效性计数：每个 gate/lint 规则记录拦截/触发次数；gate-audit 升级为 effectiveness 报告（从未触发的门要么给证据要么撤）——治理处方假设未经验证，自我度量 |

## 4. 非功能需求（五性初档，细化见 module-catalog）

- 可靠 critical（.zcode/lib 账本/门禁正确性）；安全 high（拦截不误放）；隐私 high（DENY 路径）；韧性 high（断链 fail-closed）；Safety none+reason（纯软件工具）。
