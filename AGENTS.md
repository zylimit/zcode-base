...
3. **主 Agent 唯一编排**：编码/审查/测试/部署四环节主 Agent 一律不亲自动手，只「写派单 + 委派（Agent 工具，fresh 实例）+ 验收」；子代理不再派子代理。仅文档类（Spec/CHANGELOG/DEV-PLAN/progress）可亲写（台账执法待 OQ-4 实测子代理触发域后接线）。
