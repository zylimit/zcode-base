...
把五性从口号变成可验收指标：DFX-Spec.md（人读）+ module-catalog attributes（机器执法）+ verification-matrix 认领检查。口径详见 `.zcode/rules/quality-attributes.md`。
...
2. 写 `DFX-Spec.md`（`.zcode/harness/templates/DFX-Spec-Template.md`）：每属性「档位 + 可验收指标 + 验证方式」。
3. 档位写入 module-catalog `attributes`（none/minimal 必须 reason）。
4. 每个 critical/high 属性在 verification-matrix 加认领检查（proves 该属性）——没有检查的档位是接线缺陷，`node .zcode/zbase.mjs fitness` 会拦。
