...
2. 跑 `node .zcode/zbase.mjs task status`：有活跃任务 → 检查 `baselineDrift`（true = 旧证据已腐化，受影响验证需重跑）。
3. 跑 `node .zcode/zbase.mjs fast status`：Fast 生效中 → 提醒用户到期时间，安全护栏不受影响。
4. `node .zcode/zbase.mjs risk scan`：有 critical/high 发现 → 先处理再开工。
