...
function checkFileWrite(event, input) {
  ...
  // 归一化为仓库相对路径后做 glob 匹配；绝对路径在 .zcode（含 state/）或旧装 .zbase 之下同样命中
...
function stop() {
  ...
  process.stderr.write(`[zbase Stop 门] 检测到 ${paths.length} 个未提交/未验证变更路径，且账本无覆盖当前代码状态（fingerprint）的新鲜回执。\n请完成受影响验证并落回执：node .zcode/zbase.mjs receipt write --check <name> --status PASS --note "<证据>"；或向用户说明跳过理由。续命计数 ${count}/2。\n`);
  process.exit(2);
}
