...
> 细则入口 `.zcode/rules/large-repo.md`。性能基准参考 cursor-base 同级实现实测（600k 行/30k 文件/120 模块）。
...
```bash
node .zcode/zbase.mjs impact                      # git 变更路径 → 闭包
node .zcode/zbase.mjs impact --paths src/a.ts,b/  # 指定路径
```
...
```bash
node .zcode/zbase.mjs context pack            # 默认 120K chars
node .zcode/zbase.mjs context pack --budget 80000
```
...
- 模块胶囊（`.zcode/harness/modules/<name>.md`）是 60W 行仓的上下文货币——维护胶囊比维护全量文档便宜得多。
