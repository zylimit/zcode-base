...
| 条目 | 主题 | occurrence | 毕业 |
|---|---|---|---|
| completion-claims-need-fresh-verification | 完成声明必须走五步闸（新鲜证据） | 3 | 已毕业 2026-09-08：宪法纪律5+Stop 门（P1） |
| red-locks-the-bug | 缺陷先锁失败测试再修 | 3 | 已毕业 2026-09-08：workflow 铁律+bug-fixer skill（P4） |
| three-file-sync-clearable-recap-recovery | 三文件即时同步，恢复才可靠 | 3 | 已毕业 2026-09-08：sync-check 双缝+SessionStart（P6） |
| destructive-ops-recheck-live-state | 远端/生产写操作前当场实查 | 3 | 已毕业 2026-09-08：宪法纪律8+classifier deny 档（P2） |
| main-agent-no-direct-coding | 主 Agent 只编排不编码 | 4 | 已毕业 2026-09-08：宪法纪律3/4+orchestration 三铁律（P3） |
...
| waiver-lifecycle-explicit | 临时豁免带日期定界+到期显式关闭（codex：3 条豁免 12 天后才关干净） | 3 | 已毕业 2026-09-08：waiver 五要素 expiry 到期自动失效（P7） |
...
| single-truth-source-rejection | 新状态文件先过「与账本是否两套真相」关（cc：三次同理由否决） | 3 | 已毕业 2026-09-08：ADR 真相源栏+adr check 棘轮（P5） |
...
- 已毕业条目保留原文作为根因档案；frontmatter 标 `graduated: true` + `graduatedAt` + `graduatedTo`（机制落点一句话）。新 occurrence 只更新计数。
- 2026-09-08 evolution 批 P1-P7 七条毕业标记落档（用户批准单）；P8/P9 族聚类的毕业落点在 code-review/bug-fixer skill 检查项与宪法纪律5（族内条目状态不逐条翻转，原文留档）。
- 删除/停用条目属 HIGH 审批（存量资产铁律）。
- 种子条目的 occurrence 按来源仓记录次数（Task 10.4 约定）；毕业判据=同族失败模式聚类合计 ≥3（evolution-engine），毕业须用户确认。
