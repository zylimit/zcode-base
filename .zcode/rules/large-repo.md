...
大仓治理启用 = `.zcode/harness/module-catalog.json` 存在且 `catalog lint` 通过。不存在时按普通仓库处理（不强造 catalog）；存在但 lint 报错先修 catalog 再动代码。
...
| context pack | 打包成功 | — | — | 只打印 manifest，全文落 `.zcode/state/context/` |
...
每个核心模块维护一份胶囊（`.zcode/harness/modules/<name>.md`，模板见 `.zcode/harness/templates/MODULE-CAPSULE-Template.md`）：职责/公共契约/依赖/禁边/五性档位/测试入口。context pack 优先打包胶囊而非源码全文——读胶囊不读实现，是 60W 行仓的上下文纪律。
