...
```
┌────────────────────────────────────────────────────────┐
│ 宪法层  AGENTS.md（自动注入）+ rules/ 下沉细则（指针式） │  人读的纪律
├────────────────────────────────────────────────────────┤
...
├────────────────────────────────────────────────────────┤
│ 契约层  harness/（module-catalog / verification-matrix  │  机器可读事实
│         / schemas / templates）                          │
├────────────────────────────────────────────────────────┤
│ 运行态  .zbase/（gitignored：账本/门禁日志/任务/证据）   │  会话间状态
└────────────────────────────────────────────────────────┘
```
...
1. **运行时强制 > 提示词自觉**：能进 hook/runtime 的规则不留在提示词（如危险命令、账本防篡改）。
2. **fail-visible**：断链/超时/degraded 一律显式报告，不静默降级；BLOCKED 绝不假绿。
3. **零依赖 Node**：runtime 仅用 node:* 内置模块（ADR-0003），目标项目零污染。
...
模块账本见 `harness/module-catalog.json`（7 模块：governance/skills/commands/feedback/runtime-harness/contracts/installer）。核心模块胶囊：`harness/modules/`（按需补充）。
